...
app.get("/send-newsletter/:reqId", authMiddleware, async (req, res) => {
  console.log("User email:", req.userEmail); // Access the email from the request
  const reqId = req.params.reqId;
  const page = parseInt(req.query.page) || 1;
  const limit = 1000;
  ...
  try {
      ...
      try {
        ...
        for (let i = 0; i < searchPeopleLixData?.people?.length; i++) {
          ...
          if (personaLen > 2) {
            if (!flag) {
              flag = true;
              res.redirect(`/send-newsletter/${reqUUID}`);
            }
    ...
    if (reqIdData?.syncWithGrowthx) {
      ...
      const mailOptions = {
        ...
        Subscriber link: <a href="https://growth-tool.growthx.com/send-newsletter/${reqUUID}">View Subscribers</a>.<br>
    ...
    if (!flag) res.redirect(`/send-newsletter/${reqUUID}`);
  } catch (error) {
    try {
      const csvFilePath = "people_data.csv";
      await fs.unlink(csvFilePath);
  ...
  try {
    ...
    if (reqIdData?.syncWithGrowthx) {
      ...
      const mailOptions = {
        ...
        html: `Click here to view project: <a href="https://growthx.growthx.com/projects/${projectData.data._id}">View Project</a>
        Live Newsletter management tool: <br>
        Subscriber link: <a href="https://growth-tool.growthx.com/send-newsletter/${reqUUID}">Send Newsletter</a>.<br>
        Email list link: <a href="https://growth-tool.growthx.com/get-jobs/${reqUUID}">View Email List</a><br>
        Please let us know if you have any questions on this.
        ---
        Customer Success Team`,
